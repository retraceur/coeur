...
class WP_Query {
	 ...
	 *
	 * @since WP 2.1.0
	 * @since WP 4.5.0 Removed the `comments_popup` public query variable.
	 *
	...
	public function parse_query( $query = '' ) {
				...
				if ( isset( $tax_query['operator'] ) ) {
					...
					switch ( $tax_query['taxonomy'] ) {
						...
						case 'post_tag':
							...
							break;
						case 'post_format':
							$this->is_tax = $is_tax_true || 'post-format-standard' === $query_vars['post_format'];
							break;
						default:
		...
		if ( ! ( $this->is_singular || $this->is_archive || $this->is_search || $this->is_feed
				...
				|| $this->is_404 || $this->is_admin || $this->is_robots || $this->is_favicon || $this->is_sitemap ) ) {
			$this->is_home = true;
		}
	...
	public function parse_tax_query( &$query_vars ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $query_vars[ $t->query_var ] ) ) {
				...
				if ( 'post_format' === $t->query_var ) {
					...
					if ( isset( $post_format_slugs[ $query_vars['post_format'] ] ) ) {
						$query_vars['post_format'] = $post_format_slugs[ $query_vars['post_format'] ];
		 ...
		 * it now.
		 */
		if ( isset( $query_vars['post_format'] ) && 'post-format-standard' === $query_vars['post_format'] ) {
	 ...
	 *
	 * @since WP 1.5.0
	 * @since WP 3.9.0 The `$default_value` argument was introduced.
	 *
	...
	public function get_posts() {
		...
		if ( null === $this->posts ) {
			 ...
			 *
			 * @since WP 3.4.0
			 * @since WP 6.6.0 Added the `$old_request` and `$clauses` parameters.
			 *
	 ...
	 *
	 * @since WP 4.4.0
	 * @deprecated WP 4.5.0 See wp_lazyload_comment_meta().
	 *
	 * @param mixed $check
