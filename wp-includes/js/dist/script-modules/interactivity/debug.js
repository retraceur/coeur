...
(entry, ...args) => {
  ...
  if (typeof value === 'function') {
    ...
    if (hasNegationOperator) {
      warn('Using a function with a negation operator is deprecated and will stop working in Retraceur 3.0. Please use derived state instead.');
...
function wrapEventAsync(event) {
  const handler = {
    get(target, prop, receiver) {
      ...
      switch (prop) {
        case 'currentTarget':
          warn(`Accessing the synchronous event.${prop} property in a store action without wrapping it in withSyncEvent() is deprecated and will stop working in Retraceur 3.0. Please wrap the store action in withSyncEvent().`);
          break;
        case 'preventDefault':
        case 'stopImmediatePropagation':
        case 'stopPropagation':
          warn(`Using the synchronous event.${prop}() function in a store action without wrapping it in withSyncEvent() is deprecated and will stop working in Retraceur 3.0. Please wrap the store action in withSyncEvent().`);
          break;
      }
...
const requiredConsent = 'I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of Retraceur.';
