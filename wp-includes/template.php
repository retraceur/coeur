 ...
 * @since WP 6.9.0 Added `taxonomy-{taxonomy_slug}-{term_id}.php` to the hierarchy.
 *
 * @see get_query_template()
 *
 ...
 * @since WP 7.0.6 A template name containing `..` is only located if it resolves inside the theme.
 *
 * @global string $wp_stylesheet_path Path to current theme's stylesheet directory.
 * @global string $wp_template_path   Path to current theme's template directory.
 *
 ...
 * @global string     $retraceur_version Retraceur version string.
...
function load_template( $_template_file, $load_once = true, $args = array() ) {
	global $posts, $post, $wp_did_header, $wp_query, $wp_rewrite, $wpdb, $wp_version, $retraceur_version, $wp, $id, $user_ID;
...
function wp_start_template_enhancement_output_buffer(): bool {
	...
	$started = ob_start(
		 ...
		 *
		 * The output buffer needs to be removable because Retraceur calls wp_ob_end_flush_all() and then calls
