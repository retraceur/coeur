...
function wp_register_core_abilities(): void {
	...
	$site_info_properties = array(
		...
		'url'         => array(
			...
			'description' => __( 'The public URL where visitors access the site. May differ from the Retraceur installation URL.' ),
		),
		'wpurl'       => array(
			'type'        => 'string',
			'title'       => __( 'Retraceur Address (URL)' ),
			'description' => __( 'The URL where Retraceur core files are served. May differ from the public site URL.' ),
		),
		'admin_email' => array(
			'type'        => 'string',
	...
	$user_info_properties = array(
		...
		'roles'         => array(
			'type'        => 'array',
			'title'       => __( 'Roles' ),
			'description' => __( 'Roles assigned to the user, such as administrator, contributor, or subscriber.' ),
			'items'       => array(
				'type' => 'string',
			),
		),
	...
	wp_register_ability(
		...
		array(
			...
			'description'         => __( 'Returns core details about the site\'s runtime context for diagnostics and compatibility (environment, PHP runtime, database server info, Retraceur version). By default returns all fields, or optionally a filtered subset.' ),
			'category'            => $category_site,
			'input_schema'        => array(
				'type'                 => 'object',
				'properties'           => array(
