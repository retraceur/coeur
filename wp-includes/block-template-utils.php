...
function get_default_block_template_types() {
	...
	foreach ( $post_formats as $post_format_slug => $post_format_name ) {
		$default_template_types[ 'single-post-format-' . $post_format_slug ] = array(
			'title'       => sprintf(
				/* translators: %s: Post format name. */
				_x( 'Single Post Format entries: %s', 'Template name' ),
				$post_format_name
			),
			'description' => sprintf(
				/* translators: %s: Post format name. */
				__( 'Displays the %s post format single entry.' ),
				$post_format_name
			),
		);
 ...
 *
 * @since WP 5.9.0
 * @since WP 6.3.0 Added the `$query` parameter.
 * @access private
 *
 * @param string $template_type Template type. Either 'wp_template' or 'wp_template_part'.
 * @param array  $query {
 ...
 *
 * @since WP 5.9.0
 * @since WP 6.3.0 Added `modified` property to template objects.
 * @since WP 7.1.0 Added `date` property to template objects.
 * @access private
 *
 ...
 * @since WP 5.9.0
 * @since WP 6.3.0 Added `modified` property to template objects.
 * @since WP 6.4.0 Added support for a revision post to be passed to this function.
 * @access private
 *
 * @param WP_Post $post Template post.
 * @return WP_Block_Template|WP_Error Template or error object.
 ...
 *
 * @since WP 5.9.0
 * @since WP 6.0.0 Adds the whole theme to the export archive.
 *
