...
class WP_Http {
	...
	public function request( $url, $args = array() ) {
		...
		if ( $this->block_request( $url ) ) {
			...
			$response = new WP_Error( 'http_request_not_executed', sprintf( __( 'Contributor has blocked requests through HTTP to the URL: %s.' ), $url ) );
			/** This action is documented in wp-includes/class-wp-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}
	 ...
	 *
	 * @since WP 3.2.0
	 * @deprecated WP 6.4.0 Use WpOrg\Requests\Requests::get_transport_class()
	 ...
	 *
	 * @since WP 3.2.0
	 * @deprecated WP 5.1.0 Use WP_Http::request()
	 * @see WP_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
