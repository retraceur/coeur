...
if ( ! function_exists( 'wp_mail' ) ) :
	 ...
	 *
	 * @since WP 1.2.1
	 * @since WP 5.5.0 is_email() is used for email validation,
	 *              instead of PHPMailer's default validator.
	 * @since WP 6.9.0 The `$embeds` parameter was added.
	 * @since WP 6.9.0 Improved Content-Type header handling for multipart messages.
	 *
	...
	function wp_mail( $to, $subject, $message, $headers = '', $attachments = array(), $embeds = array() ) {
		 ...
		 *
		 * @since WP 2.2.0
		 * @since WP 6.9.0 The `$embeds` element was added to the `$args` array.
		 *
		 * @param array $args {
		 ...
		 *
		 * @since WP 5.7.0
		 * @since WP 6.9.0 The `$embeds` element was added to the `$atts` array.
		 *
		...
		try {
			 ...
			 *
			 * @since WP 5.9.0
			 * @since WP 6.9.0 The `$embeds` element was added to the `$mail_data` array.
			 *
...
if ( ! function_exists( 'wp_authenticate' ) ) :
	...
	function wp_authenticate(
		...
		if ( is_wp_error( $user ) && ! in_array( $user->get_error_code(), $ignore_codes, true ) ) {
			 ...
			 * @since WP 5.4.0 The `$error` parameter was added.
			 *
			 * @param string   $username Username or email address.
...
if ( ! function_exists( 'wp_logout' ) ) :
	...
	function wp_logout() {
		 ...
		 *
		 * @since WP 1.5.0
		 * @since WP 5.5.0 Added the `$user_id` parameter.
		 *
...
if ( ! function_exists( 'wp_parse_auth_cookie' ) ) :
	 ...
	 *
	 * @since WP 2.7.0
	 * @since WP 4.0.0 The `$token` element was added to the return value.
	 *
	 * @param string $cookie Authentication cookie.
	 * @param string $scheme Optional. The cookie scheme to use: 'auth', 'secure_auth', or 'logged_in'.
	 * @return string[]|false {
...
if ( ! function_exists( 'wp_set_auth_cookie' ) ) :
	...
	function wp_set_auth_cookie( $user_id, $remember = false, $secure = '', $token = '' ) {
		 ...
		 *
		 * @since WP 4.7.4
		 * @since WP 6.2.0 The `$expire`, `$expiration`, `$user_id`, `$scheme`, and `$token` parameters were added.
		 *
...
if ( ! function_exists( 'check_admin_referer' ) ) :
	 ...
	 *
	 * @since WP 1.2.0
	 * @since WP 2.5.0 The `$query_arg` parameter was added.
	 *
...
if ( ! function_exists( 'wp_redirect' ) ) :
	 ...
	 *
	 * @since WP 1.5.1
	 * @since WP 5.1.0 The `$x_redirect_by` parameter was added.
	 * @since WP 5.4.0 On invalid status codes, wp_die() is called.
	 *
	 * @global bool $is_IIS
	 *
...
if ( ! function_exists( 'wp_safe_redirect' ) ) :
	 ...
	 *
	 * @since WP 2.3.0
	 * @since WP 5.1.0 The return value from wp_redirect() is now passed on, and the `$x_redirect_by` parameter was added.
	 *
	 * @param string       $location      The path or URL to redirect to.
	 * @param int          $status        Optional. HTTP response status code to use. Default '302' (Moved Temporarily).
	 * @param string|false $x_redirect_by Optional. The application doing the redirect or false to omit. Default 'Retraceur'.
...
if ( ! function_exists( 'wp_nonce_tick' ) ) :
	 ...
	 *
	 * @since WP 2.5.0
	 * @since WP 6.1.0 Added `$action` argument.
	 *
	...
	function wp_nonce_tick( $action = -1 ) {
		 ...
		 *
		 * @since WP 2.5.0
		 * @since WP 6.1.0 Added `$action` argument to allow for more targeted filters.
		 *
...
if ( ! function_exists( 'wp_hash' ) ) :
	 ...
	 *
	 * @since WP 2.0.3
	 * @since WP 6.8.0 The `$algo` parameter was added.
	 *
...
if ( ! function_exists( 'wp_hash_password' ) ) :
	 ...
	 *
	 * @since WP 2.5.0
	 * @since WP 6.8.0 The password is now hashed using bcrypt by default instead of phpass.
	 *
...
if ( ! function_exists( 'wp_check_password' ) ) :
	 ...
	 *
	 * @since WP 2.5.0
	 * @since WP 6.8.0 Passwords in Retraceur are now hashed with bcrypt by default. A
...
if ( ! function_exists( 'wp_password_needs_rehash' ) ) :
	 ...
	 * of Retraceur may still be hashed with phpass and will need to be rehashed. If the default cost
	 * or algorithm is changed in PHP or Retraceur then a password hashed in a previous version will
	 * need to be rehashed.
	 *
...
if ( ! function_exists( 'wp_generate_password' ) ) :
	...
	function wp_generate_password( $length = 12, $special_chars = true, $extra_special_chars = false ) {
		 ...
		 *
		 * @since WP 3.0.0
		 * @since WP 5.3.0 Added the `$length`, `$special_chars`, and `$extra_special_chars` parameters.
		 *
...
if ( ! function_exists( 'wp_rand' ) ) :
	 ...
	 * @since WP 6.1.0 Returns zero instead of a random number if both `$min` and `$max` are zero.
	 *
...
if ( ! function_exists( 'wp_set_password' ) ) :
	...
	function wp_set_password(
		 ...
		 *
		 * @since WP 6.2.0
		 * @since WP 6.7.0 The `$old_user_data` parameter was added.
		 *
...
if ( ! function_exists( 'get_avatar' ) ) :
	 ...
	 * @since WP 5.5.0 Added the `loading` argument.
	 * @since WP 6.1.0 Added the `decoding` argument.
	...
	function get_avatar( $id_or_email, $size = 96, $default_value = '', $alt = '', $args = null ) {
		 ...
		 *
		 * @since WP 2.5.0
		 * @since WP 4.2.0 Added the `$args` parameter.
		 *
