 ...
 *
 * @param string $meta_type  Type of object metadata is for. Accepts 'blog', 'post', 'term',
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	 ...
	 * (blog, post, term, user, or any other type with an associated meta table).
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	 ...
	 * @param string $meta_type Type of object metadata is for. Accepts 'blog', 'post', 'term', 'user',
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		if ( 'post' === $meta_type ) {
			/**
			 * Fires immediately before deleting post metadata of a specific type.
			 *
			 * The dynamic portion of the hook name, `$meta_type`, refers to the post meta
			 * object type.
			 *
			 * Possible hook names include:
			 *
		...
		if ( 'post' === $meta_type ) {
			/**
			 * Fires immediately after deleting post metadata of a specific type.
			 *
			 * The dynamic portion of the hook name, `$meta_type`, refers to the post meta
			 * object type.
			 *
 ...
 * @global wpdb $wpdb WP database abstraction object.
 *
 * @param string $type Type of object metadata is for. Accepts 'blog', 'post', 'term',
 *                     'user', or any other object type with an associated meta table.
 ...
 *
 * @param string $meta_key  Metadata key.
 * @param string $meta_type Optional. Type of object metadata is for. Accepts 'blog', 'post', 'term',
 ...
 * @param string $object_type    Type of object metadata is for. Accepts 'blog', 'post', 'term',
 *                               'user', or any other object type with an associated meta table.
...
function sanitize_meta( $meta_key, $meta_value, $object_type, $object_subtype = '' ) {
	if ( ! empty( $object_subtype ) && has_filter( "sanitize_{$object_type}_meta_{$meta_key}_for_{$object_subtype}" ) ) {
		 ...
		 * and `$object_subtype`, refer to the metadata object type (blog, post, term, or user),
	 ...
	 * refer to the metadata object type (blog, post, term, or user) and the meta
 ...
 *
 * If an object type does not support any subtypes, such as blogs or users you should commonly call this function
