 ...
 *
 * @since WP 6.8.0
 * @since WP 7.1.0 The `WP_SPECULATIVE_LOADING_DEFAULT_MODE` and `WP_SPECULATIVE_LOADING_DEFAULT_EAGERNESS` constants and
...
function wp_get_speculation_rules_configuration(): ?array {
	 ...
	 *
	 * By default, the speculation rules configuration is decided by WP Core ("auto"). This filter can be used
	...
	if (
		...
		// 'immediate' is a valid eagerness, but for safety WP does not allow it for document-level rules.
 ...
 *
 * Retraceur Core defaults to a mode of 'prefetch' and an eagerness of 'conservative'. Hosting providers can override
 ...
 * precedence.
 *
 * Note that an eagerness of 'immediate' is not permitted as a default, since Retraceur does not allow it for the
...
function wp_get_speculation_rules_default_configuration(): array {
	...
	if (
		...
		// 'immediate' is a valid eagerness, but for safety Retraceur does not allow it for document-level rules.
		'immediate' !== $eagerness
	) {
 ...
 *
 * Additional speculation rules other than the default rule from WP Core can be provided by using the
...
function wp_get_speculation_rules(): ?WP_Speculation_Rules {
	 ...
	 * If the WP site is in a subdirectory, the exclude paths will automatically be prefixed as necessary.
	 *
	 * Note that WP always excludes certain path patterns such as `/wp-login.php` and `/wp-admin/*`, and those
