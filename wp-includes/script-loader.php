...
function wp_default_scripts( $scripts ) {
	...
	if ( is_admin() ) {
		...
		$scripts->add( 'plugin-install', "/wp-admin/js/plugin-install$suffix.js", array( 'wp-dom-ready' ), false, 1 );
		...
		$scripts->add( 'dashboard', "/wp-admin/js/dashboard$suffix.js", array( 'common', 'jquery', 'postbox', 'wp-util', 'wp-a11y', 'wp-date' ), false, 1 );
...
function wp_enqueue_global_styles() {
	 ...
	 * demand is disabled (which is no longer the default since WP 6.9).
	 */
	if (
 ...
 *
 * @since WP 5.7.0
 * @since WP 7.0.0 Returns an empty string if the data cannot be safely embedded in a script tag.
 *
...
function wp_maybe_inline_styles() {
	 ...
	 *
	 * @since WP 5.8.0
	 * @since WP 6.9.0 The default limit increased from 20K to 40K.
	 *
 ...
 *
 * @since WP 5.9.1
 * @since WP 6.1.0 Added the `$priority` parameter.
 *
 ...
 *
 * @since WP 6.1.0
 * @since WP 6.8.0 Moved stylesheet registration outside of this function.
 */
 ...
 *
 * @since WP 6.9.0
 * @since WP 7.0.0 This is now invoked at the `wp_default_styles` action with priority 0 instead of at `init` with priority 8.
 *
