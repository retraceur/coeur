...
function login_header( $title = null, $message = '', $wp_error = null ) {
	...
	$login_header_url = get_bloginfo( 'url', 'display' );;
	 ...
	 *
	 * @since WP 2.1.0
	 * @deprecated WP 5.2.0 Use {@see 'login_headertext'} instead.
	 *
	...
	$login_header_text = empty( $login_header_title ) ? __( 'With ♥ from Retraceur' ) : $login_header_title;
	...
	$classes = array( 'login-action-' . $action, 'wp-core-ui', 'admin-color-retraceur' );
...
switch ( $action ) {
	...
	case 'retrievepassword':
		 ...
		 *
		 * @since WP 1.5.1
		 * @since WP 5.1.0 Added the `$errors` parameter.
		 *
		 * @param WP_Error $errors A `WP_Error` object containing any errors generated by using invalid
		 *                         credentials. Note that the error object may not contain any errors.
		 */
	...
	case 'register':
		 ...
		 *
		 * @since WP 3.0.0
		 * @since WP 5.9.0 Added the `$errors` parameter.
		 *
