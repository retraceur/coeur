...
$user_roles_help = array_intersect_key(
	array(
		'subscriber'    => __( 'Subscribers can read regular site content but cannot create it.' ),
		'contributor'   => __( 'Contributors can write and manage their posts but not publish posts or upload media files.' ),
		'author'        => __( 'Authors can publish and manage their own posts, and are able to upload files.' ),
		'editor'        => __( 'Editors can publish posts, manage posts as well as manage other people&#8217;s posts, etc.' ),
		'administrator' => __( 'Administrators have access to all the administration features.' ),
	),
	get_editable_roles()
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' => '<p>' . __( 'Here is a basic overview of the different contributor roles and the permissions associated with each one:' ) . '</p>' .
							'<ul><li>' . implode( '</li><li>', $user_roles_help ) . '</li></ul>',
	)
);
