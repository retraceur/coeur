	...
	if ( $posts->have_posts() ) {
		...
		while ( $posts->have_posts() ) {
			...
			if ( has_post_format( get_supported_post_format_slugs(), $post ) ) {
				$post_title = sprintf(
					/* Translators: 1: is the Post ID. 2: the Post Format */
					esc_html__( '#%1$s %2$s' ),
					$post->ID,
					_post_states( $post, false )
				);
			} else {
			...
			printf(
				...
				esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;' ), $post_title ) ),
				$post_title
			);
		}
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php if ( $is_block_theme ) : ?>
				...
				<a href="<?php echo esc_url( add_query_arg( 'p', rawurlencode( '/styles' ), admin_url( 'site-editor.php' ) ) ); ?>"><?php esc_html_e( 'Edit styles' ); ?></a>
			<?php else : ?>
