...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				if ( 'blocks' === $this->_args['plural'] ) {
					echo ' <a href="' . esc_url( admin_url( 'block-install.php' ) ) . '">' . __( 'Search for blocks in the Retraceur Block Directory.' ) . '</a>';
				} else {
					echo ' <a href="' . esc_url( admin_url( 'plugin-install.php' ) ) . '">' . __( 'Search for plugins in the Retraceur Plugin Directory.' ) . '</a>';
				}
			}
	...
	public function single_row( $item ) {
		...
		// It's not a plugin the author tested with Retraceur.
		if ( ! $requires_r ) {
			printf(
				'<tr class="plugin-update-tr"><td colspan="%s" class="plugin-update colspanchange">',
				esc_attr( $this->get_column_count() )
