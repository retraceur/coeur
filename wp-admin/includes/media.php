<?php
/**
 * Retraceur Administration Media API.
 *
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	 ...
	 *
	 * @since WP 2.5.0
	 * @since WP 5.6.0 The `$rel` parameter was added.
	 *
...
function wp_iframe( $content_func, ...$args ) {
	...
	?>
	<title><?php bloginfo( 'name' ); ?> &rsaquo; <?php _e( 'Uploads' ); ?> &#8212; <?php _e( 'Retraceur' ); ?></title>
	<?php
	...
	<body<?php echo $body_id_attr; ?> class="wp-core-ui no-js <?php echo 'admin-color-' . sanitize_html_class( get_user_option( 'admin_color' ), 'retraceur' ); ?>">
	<script>
 ...
 *
 * @since WP 2.6.0
 * @since WP 4.2.0 Introduced the `$return_type` parameter.
 * @since WP 4.8.0 Introduced the 'id' option for the `$return_type` parameter.
 * @since WP 5.3.0 The `$post_id` parameter was made optional.
 * @since WP 5.4.0 The original URL of the attachment is stored in the `_source_url`
 *              post meta value.
 * @since WP 5.8.0 Added 'webp' to the default list of allowed file extensions.
 *
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . esc_html__( 'The web browser on your device cannot be used to upload files.' ) . '</p>';
		return;
	}
	...
	*/
	...
	<div id="drag-drop-area">
		...
		<p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php esc_attr_e( 'Select Files' ); ?>" class="button button-hero" /></p>
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
		<?php
			printf(
				/* translators: %s: HTML attributes for button. */
				__( 'You are using the browser&#8217;s built-in file uploader. The Retraceur uploader includes multiple file selection and drag and drop capability. <button %s>Switch to the multi-file uploader</button>.' ),
				'type="button" class="button-link"'
			);
		?>
	</p>
