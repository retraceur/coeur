...
function _wp_translate_postdata( $update = false, $post_data = null ) {
	...
	if ( $update && ! current_user_can( 'edit_post', $post_data['ID'] ) ) {
		...
		if ( 'page' === $post_data['post_type'] ) {
			return new WP_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to create pages as this contributor.' ) );
		} else {
			return new WP_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to create posts as this contributor.' ) );
		}
	}
...
function _admin_notice_post_locked() {
	...
	if ( $locked ) {
		 ...
		 *
		 * @since WP 3.6.0
		 * @since WP 5.4.0 The $user parameter was added.
		 *
		 * @param WP_Post $post Post object.
		 * @param WP_User $user The user with the lock for the post.
		 */
...
function wp_create_post_autosave( $post_data ) {
	...
	if ( $old_autosave ) {
		 ...
		 *
		 * @since WP 4.1.0
		 * @since WP 6.4.0 The `$is_update` parameter was added to indicate if the autosave is being updated or was newly created.
		 *
 ...
 * @since WP 6.4.0 Added `block_hooks` field.
 *
