<?php
/**
 * Class for looking up a site's health based on a user's Retraceur environment.
 *
 * @since WP 5.2.0
...
class WP_Site_Health {
	...
	/**
	 * Tests for Retraceur version and outputs it.
	 *
	 ...
	 *
	 * @since WP 5.2.0
	 * @since WP 5.3.0 The `$constant_name` and `$class_name` parameters were added.
	 *
	 ...
	 *
	 * This test builds on the recommendations made by the Hosting Team.
	 *
	 * @since WP 5.2.0
	 *
	 * @return array
	...
	public function get_test_php_extensions() {
		$result = array(
			...
			'description' => __( 'PHP modules perform most of the tasks on the server that make your site run. Any changes to these must be made by your server administrator.' ),
			'actions'     => '',
			'test'        => 'php_extensions',
		);
		 ...
		 *
		 * @since WP 5.2.0
		 * @since WP 5.3.0 The `$constant` and `$class` parameters were added.
		 *
		 * @param array $modules {
			...
			if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( 'The value, %s, has been added to this website&#8217;s configuration file. This means any errors on the site will be written to a file which is potentially available to everyone.' ),
	 ...
	 *
	 * @since WP 5.2.0
	 * @since WP 5.7.0 Updated to rely on {@see wp_is_using_https()} and {@see wp_is_https_supported()}.
	 *
	 * @return array The test results.
	 */
	 ...
	 *
	 * A loopback is when Retraceur queries itself, for example to start a new WP_Cron instance,
	...
	public function get_test_page_cache() {
		$description  = '<p>' . __( 'Page cache enhances the speed and performance of your site by saving and serving static pages instead of calling for a page every time someone visits.' ) . '</p>';
	...
	public static function get_tests() {
		 ...
		 * both the Retraceur Hosting Team and web standards in general.
		 *
		 ...
		 *
		 * @since WP 5.2.0
		 * @since WP 5.6.0 Added the `async_direct_test` array key for asynchronous tests.
	 ...
	 *
	 * Loopbacks are what Retraceur uses to communicate with itself to start up WP_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		 ...
		 * GET request Retraceur uses for wp-cron.php loopback requests but is close
		 * enough.
		 */
	...
	public function get_page_cache_headers(): array {
		 ...
		 * or not. Retraceur checks for each of the headers in the returned array. If the callback is provided, it will
		 * be passed the value for the corresponding header and return a boolean value indicating if the header suggests
		 * that a cache is active. If the value is `null` for the header, then Retraceur will assume that a cache is
