...
$profile_help = '<p>' . __( 'Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using Retraceur.' ) . '</p>' .
	'<p>' . __( 'You can change your password, turn on keyboard shortcuts, change the color scheme of your Retraceur administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.' ) . '</p>' .
	'<p>' . __( 'You can select the language you wish to use while using the Retraceur administration screen without affecting the language site visitors see.' ) . '</p>' .
		...
		<div class="wrap" id="profile-page">
			...
			<?php if ( ! IS_PROFILE_PAGE ) : ?>
				<?php if ( current_user_can( 'create_users' ) ) : ?>
					<a href="user-new.php" class="page-title-action"><?php echo esc_html__( 'Add Contributor' ); ?></a>
			...
			<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'profile.php' : 'user-edit.php' ) ); ?>" method="post" novalidate="novalidate"
				...
				<table class="form-table" role="presentation">
					...
					<tr class="user-admin-color-wrap">
						<th scope="row"><?php esc_html_e( 'Administration Color Scheme' ); ?></th>
						<td>
							<?php
							/**
								...
								<p class="description">
									...
									if ( IS_PROFILE_PAGE ) {
										$description = sprintf(
											...
											__( '<a href="%s">You can change your profile picture on Libravatar</a>.' ),
											/* translators: The localized Gravatar URL. */
											__( 'https://libravatar.org/' )
										);
									} else {
										$description = '';
									}
